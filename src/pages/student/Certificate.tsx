...
export default function Certificate() {
  ...
  const validationCode = btoa(`${user.id}-${course.id}`).substring(0, 10).toUpperCase()
      ...
      <Card className="border-[12px] border-double border-primary/20 aspect-[1.414/1] bg-white text-slate-900 flex items-center justify-center relative overflow-hidden shadow-2xl">
        ...
        <CardContent className="text-center space-y-8 relative z-10 p-16 max-w-4xl w-full">
          ...
          <div className="flex justify-between items-end mt-24 text-slate-500 font-medium px-12 pt-12">
            ...
            <div className="text-center text-sm opacity-60 w-1/3 flex flex-col items-center pb-2">
              <p>Código de Validação</p>
              <p className="font-mono mt-1 px-3 py-1 bg-slate-100 rounded tracking-widest">
                {validationCode}
              </p>
            </div>
